...
class ChatCompletionChoiceDelta(BaseModel):
    ...
    function_call: Optional[ChatCompletionChoiceDeltaFunctionCall] = None
    ...
    def role_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["system", "user", "assistant", "function", "tool"]):
            raise ValueError(
                "must be one of enum values ('system', 'user', 'assistant', 'function', 'tool')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "tool_calls": (
                    [ToolChoice.from_dict(_item) for _item in obj["tool_calls"]]
                    if obj.get("tool_calls") is not None
                    else None
                ),
                "function_call": (
                    ChatCompletionChoiceDeltaFunctionCall.from_dict(
                        obj["function_call"]
                    )
